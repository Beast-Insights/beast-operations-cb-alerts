...
>(({ className, size = 'default', ...props }, forwardedRef) => {
  const sizeClasses = size === 'small'
    ? { root: 'h-4 w-7', thumb: 'h-3 w-3 data-[state=checked]:translate-x-3 data-[state=unchecked]:translate-x-0' }
    : { root: 'h-5 w-9', thumb: 'h-4 w-4 data-[state=checked]:translate-x-4 data-[state=unchecked]:translate-x-0' };

  return (
    <SwitchPrimitives.Root
      ref={forwardedRef}
      className={cx(switchStyles.root, sizeClasses.root, className)}
      {...props}
    >
      <SwitchPrimitives.Thumb className={cx(switchStyles.thumb, sizeClasses.thumb)} />
    </SwitchPrimitives.Root>
  );
});
