...
export default function OverviewPage() {
  ...
  const fetchData = useCallback(async (manual: boolean) => {
    ...
    try {
      const res = await fetch(`/api/recon/overview${manual ? '?refresh=1' : ''}`, { cache: 'no-store' })
      ...
      {data.banner && (
        ...
        >
          <div className="flex items-start justify-between gap-4">
            ...
            <Link href="/recon/scrapers?status=error" className="inline-flex shrink-0 items-center gap-1 text-sm font-medium underline-offset-2 hover:underline">
...
function KpiActive({ data }: { data: OverviewResponse }) {
  ...
  return (
    <Card className="p-4">
      ...
      <Link href="/recon/scrapers" className="mt-3 inline-flex items-center gap-1 text-xs font-medium text-blue-600 hover:underline dark:text-blue-400">
          ...
          return (
            <li key={c.client_id}>
              <div className="flex items-center justify-between text-[11px]">
                <Link href={`/recon/scrapers?client=${encodeURIComponent(c.client_id)}`} className="font-medium tabular-nums text-gray-700 hover:underline dark:text-gray-300">
                  {c.client_id}
                </Link>
